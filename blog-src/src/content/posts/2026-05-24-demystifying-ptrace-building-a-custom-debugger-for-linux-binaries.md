...
*   **`PTRACE_DETACH`**: Cleanly disconnects the tracer from the tracee and resumes its execution from wherever it was stopped. If you forget to call it, that's not actually the trap it sounds like: per `ptrace(2)`'s "Death of the tracer" behavior, if the tracer process itself exits — including crashing — the kernel automatically detaches all of its tracees and resumes them (the one exception is a tracee that's in a group-stop at that moment, which waits for a `SIGCONT` rather than resuming immediately). So a well-behaved `PTRACE_DETACH` is still the clean way to end a session, but a debugger that crashes without calling it doesn't leave the tracee stuck — the kernel cleans up after you.
