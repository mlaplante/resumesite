...
*   **Cleanup:** Always remember to disable and remove any kprobes and Ftrace filters you've added once your investigation is complete. Leftover probes continue consuming kernel resources indefinitely and can pollute future traces with noise from a debugging session nobody remembers starting. `echo > /sys/kernel/debug/tracing/kprobe_events` clears every dynamically added kprobe at once, `echo nop > /sys/kernel/debug/tracing/current_tracer` resets the active tracer, and clearing `set_ftrace_filter` (`echo > /sys/kernel/debug/tracing/set_ftrace_filter`) removes any function filters you applied.
*   **Kernel Lockdown:** On systems booted with Secure Boot and running in lockdown mode, several of the raw `debugfs` write paths used throughout this post are restricted or disabled outright, and some `kprobes` placements are refused for functions the kernel considers security-sensitive. If a probe or filter write fails with `-EPERM` and the syntax looks correct, check `/sys/kernel/security/lockdown` before assuming you made a mistake.

## Conclusion

Ftrace and KProbes turn the kernel from a black box into something you can actually observe while it's running, which is invaluable when a recompile-and-reboot debugging loop simply isn't an option. Start broad with Ftrace's `function_graph` tracer to find where time or calls are concentrated, then narrow in with targeted KProbes to pull out the specific arguments or data that confirm your hypothesis. Used deliberately, with an eye toward overhead and cleanup, these tools will get you to the bottom of kernel-level mysteries far faster than staring at `dmesg` output ever will.