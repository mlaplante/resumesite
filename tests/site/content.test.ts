...
describe('blog content completeness', () => {
  // Every post must read as finished. The generator used to publish bodies cut
  // off by Gemini's output-token cap (it never checked `finishReason`), which
  // shipped 65 posts ending mid-sentence before anyone noticed. All of them have
  // been repaired, so this gate is unconditional — there is nothing to
  // grandfather, and a failure here means a truncated post got through again.
  it.each(files)('%s is not truncated', (file) => {
    const raw = readFileSync(join(POSTS_DIR, file), 'utf8');
    const reason = findTruncation(raw.replace(/^---\n[\s\S]*?\n---\n/, ''));
