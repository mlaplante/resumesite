...
| `GEMINI_FALLBACK_MODELS` | shell env (optional) | `scripts/generate-post-gemini.js:21` | `'gemini-3.6-flash,gemini-2.5-flash-lite'` | n/a — comma-separated fallback chain. Retryable errors (429/500/502/503/504) retry then advance; 400/404 (model retired or rejects the request shape) advance immediately; 401/403 abort. |
| `GEMINI_EMBED_MODEL` | shell env (optional) | `scripts/generate-post-gemini.js:24` | `'gemini-embedding-001'` | n/a — used for semantic dedupe embeddings. Cache keys are namespaced by this model. |
...
| `_typos.toml` extend-words | `aks`, `cose`, `counterfit`, `fpr`, `hashi`, `hav`, `parm`, `shure`, `sie`, `unparseable` | `_typos.toml:12-21` | Spell-check allowlist for `crate-ci/typos` (`typos.yml` CI job). Add an entry here (with a one-line justification comment) rather than rewording copy around a false positive. |
